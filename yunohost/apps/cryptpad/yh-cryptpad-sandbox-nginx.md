# CryptPad on YunoHost: Sandbox Domain nginx Fix
...
The manually created sandbox config is overwritten or missing in three situations:
...
A systemd timer therefore checks every 30 seconds whether the config is correct and restores it automatically if needed.
...
YunoHost overwrites nginx configurations during updates and `regen-conf` calls. To prevent this, create a hook that automatically recreates the file after every `regen-conf`:
...
### Step 9: Create a Timer — Automatically Restore Configuration After Domain Actions

When a domain is added or removed in YunoHost, the sandbox config gets overwritten. The timer checks every 30 seconds whether the config is correct and restores it automatically if needed.
...
[Service]
Type=oneshot
ExecStart=/bin/bash -c 'grep -q "listen 443" /etc/nginx/conf.d/sandbox.cryptpad.example.com.conf || (bash /usr/share/yunohost/hooks/conf_regen/99-nginx_sandbox_cryptpad post nginx && systemctl reload nginx)'
```
> **Note:** Replace `sandbox.cryptpad.example.com` with your own domain from Step 2.
...
> **Note:** After a domain action in YunoHost, it may take up to 30 seconds for CryptPad to fully recover.
...
### Step 11: Verify CryptPad Functionality
...
3. **Overwriting during domain actions:** Every time a domain is added or removed in YunoHost, YunoHost runs `regen-conf` and reloads nginx multiple times. This resets the sandbox config to the port 80 block only. The hook recreates it, but YunoHost reloads nginx again afterwards — leaving the config without the port 443 block.

### Workaround

The workaround consists of four parts:

**Part 1:** Manually deploy the template file (one-time).

**Part 2:** A `conf_regen` hook at `/usr/share/yunohost/hooks/conf_regen/99-nginx_sandbox_cryptpad` recreates the config after every `regen-conf`. Uses only `nginx -t` without reload.

**Part 3:** A systemd service at `/etc/systemd/system/cryptpad-sandbox-nginx.service` restores the config after every reboot. Starts after nginx (`After=nginx.service`) and reloads nginx via `ExecStartPost`.

**Part 4:** A systemd timer at `/etc/systemd/system/cryptpad-sandbox-nginx-check.timer` checks every 30 seconds whether the config contains the port 443 block. If missing, it recreates the config and reloads nginx. Catches the cases where YunoHost domain actions overwrite the hook's result.

### Recommendation for Package Maintainers

The CryptPad app install script should deploy `nginx-sandbox.conf` analogously to the main domain configuration. The sandbox domain should either be registered as a full YunoHost domain, or a `conf_regen` hook should be shipped directly within the app package.
