...
In der CryptPad-Oberfläche erscheint beim Öffnen von Dokumenten (Tabellen, Präsentationen, Code-Editor, Umfragen etc.) ein endloser Ladebalken. Nur der "Write"-Editor (einfacher Texteditor) funktioniert – alle anderen Dokumenttypen hängen bei "Loading".
...
### Schritt 7: Dauerhafte Absicherung mit einem Hook
...
### Schritt 8: Hook testen

Prüfen ob der Hook korrekt funktioniert:
...
Wenn `HTTP/2 200` zurückkommt, ist alles korrekt konfiguriert. ✅

Die Meldung `expected to be deleted but was kept back` ist dabei normal und kann ignoriert werden – sie bedeutet nur, dass YunoHost die Datei nicht selbst verwaltet, der Hook sie aber nach jedem Löschen sofort neu erstellt.

---

### Schritt 9: CryptPad testen
...
**Das Problem:** Diese Template-Datei wird beim Installationsprozess nicht in `/etc/nginx/conf.d/` deployed. Das Installationsskript (`install` script der App) ruft zwar `ynh_add_nginx_config` auf, aber offenbar nur für die Hauptdomain-Konfiguration – nicht für `nginx-sandbox.conf`.
...
Der in dieser Anleitung beschriebene Workaround besteht aus zwei Teilen:

**Teil 1:** Manuelles Deployen der Template-Datei:
```bash
sed -e 's/__DOMAIN__/cryptpad.beispiel.de/g' \
    -e 's/__APP__/cryptpad/g' \
    -e 's/__PORT__/3000/g' \
...
**Teil 2:** Ein `conf_regen`-Hook unter `/usr/share/yunohost/hooks/conf_regen/99-nginx_sandbox_cryptpad` stellt sicher, dass die Datei nach jedem `yunohost tools regen-conf nginx` Aufruf (z.B. bei YunoHost-Updates) automatisch neu erstellt wird.

Der Hook folgt der YunoHost-Hook-Konvention mit `do_pre_regen`, `do_init_regen` und `do_post_regen` Funktionen und wird über `"do_$1_regen"` aufgerufen.
